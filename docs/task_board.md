...
| 2026-02-26 | Release gates regressed: `quality-gates` and Linux video smoke in `cuda-cross-platform` failed on `main` | Platform Engineer | Relax video-smoke quality thresholds for CI robustness; add `numpy` to `requirements-dev.txt`; harden coverage gate install/retry path; collect two fresh green runs | in_progress |
